...
use euphoxide_bot::{Instance, InstanceConfig, InstanceEvent};
...
async fn run() -> anyhow::Result<()> {
    let config = InstanceConfig::new("test").with_username("examplebot");
    let (_, mut event_rx) = Instance::new((), config);
